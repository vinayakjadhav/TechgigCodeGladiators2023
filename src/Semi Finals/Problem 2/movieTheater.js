...
process.stdin.on('end', function() {
  ...
  R = +input_stdin_array[0].split(" ")[1];
  input_currentline = 1;

  for (var i = 0; i < R; i++) {
    var line = input_stdin_array[input_currentline++].split(" ");

    costsGraphMatrix[+line[0] - 1] = costsGraphMatrix[+line[0] - 1] || new Array(H).fill(0);
    costsGraphMatrix[+line[1] - 1] = costsGraphMatrix[+line[1] - 1] || new Array(H).fill(0);
    costsGraphMatrix[+line[0] - 1][+line[1] - 1] = +line[2];
    costsGraphMatrix[+line[1] - 1][+line[0] - 1] = +line[2];

    if ((line[0] == H) || (line[1] == H)) {
      roadExist = true;
    }
  }
  // console.log("H, R ", H, R);
  ...
  var costFare;
  if (roadExist)
    costFare = (dijkstra(costsGraphMatrix, 0))[H - 1];

  // console.log("costFare ", costFare);
